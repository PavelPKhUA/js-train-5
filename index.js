...
function multiplyArray(array) {
  let dobutok = 1 // Ініціалізуємо змінну для зберігання добутку
  for (const item of array) {
    // Проходимося по всіх значеннях масиву
    dobutok *= item // Множимо кожен елемент на загальний добуток
  }
  return dobutok // Повертаємо добуток
}
...
function printUppercase(array) {
  for (const item of array) {
    ...
    console.log(item.toUpperCase()) // Перетворюємо кожен елемент в верхній регістр та виводимо на консоль
  }
}
...
function printStrings(array) {
  console.log(array.toString().trim().repeat(2)) // Перетворюємо масив в рядок і видаляемо пробіли та виводимо його на консоль
}
...
function concatenateStrings(array, subString) {
  ...
  let isIncludes = str.includes(subString) //Перевіряємо чи містить рядок підрядок
  console.log(isIncludes) // Виводимо результат
}
...
function addTenToEach(array) {
  const newArray = Array(array.length) // Ініціалізуємо масив через конструктор з такою же кількістю елементів як в вхідному масиві для зберігання результату
  for (const index in array) {
    // Перебираємо масив за допомогою циклу for-in
    newArray[index] = array[index] + 10 // Додаємо до поточного елемента 10 та додаємо результат в новий масив
  }
  console.log(newArray) // Виводимо результат
}
...
function getEvenNumbers(arr) {
  const newArray = [] // Створюємо пустий масив для зберігання результату
  for (const item of arr) {
    // Проходимо через кожен елемент вхідного масиву
    if (item % 2 === 0) {
      // Перевіряємо чи є число парним
      newArray[newArray.length] = item // Якщо так, додаємо його до масиву результату
    }
  }
  return newArray // Повертаємо масив результату
}
console.log(
  'Завдання 11 ====================================',
)
...
function multiplyByIndex(arr) {
  ...
  for (const index in arr) {
    // Проходимо через кожен елемент вхідного масиву
    newArray[index] = arr[index] * index // Помножуємо число на його індекс і додаємо до масиву результату
  }
  return newArray // Повертаємо масив результату
}
console.log(
...
function replaceNumbers(arr) {
  ...
  for (const item of arr) {
    ...
    if (item > 10) {
      ...
      newArray[newArray.length] = 'Greater than 10' // Якщо число більше за 10, додаємо рядок "Greater than 10" до масиву результату
    } else if (item <= 10) {
